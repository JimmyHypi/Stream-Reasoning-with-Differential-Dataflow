...
fn main(){
    ...
    let worker_guard = 
        timely::execute_from_args(std::env::args(), move |worker| {
            ...
            let result = reasoning_service::return_vector(&mut result_trace, 1);
            // again we are considering only worker 0 should change
            if index == 0 {
                println!("Saving full materializaton to vector: {}ms", timer.elapsed().as_millis());
            }
    ...
    let full_materialization_output_path = format!("C:\\Users\\xhimi\\Documents\\University\\THESIS\\data\\LUBM\\LUBM{}\\result\\full_materialization\\lubm{}_full_peers{}.nt", universities, universities, peers);
    
    let timer = Instant::now();
    reasoning_service::save_concatenate(&full_materialization_output_path, result);
    // To obtaint the actual time to save we should sum the previous with this
    println!("Saving to file full materialization: {}ms", timer.elapsed().as_millis());
